...
class Career
{
    ...
    public function getCareers($id = null)
    {
        ...
        if (isset($this->connection)) {
            if (empty($id)) {
                return $this->connection->select("ASSOCIATIVE", "career","id, name, SEC_TO_TIME(total_duration*60) as total_duration_formatted, total_duration");
            } else {
                return $this->connection->select("ASSOCIATIVE", "career","name, SEC_TO_TIME(total_duration*60) as total_duration_formatted, total_duration",FALSE,"id", $id, "=");
            }
        }
        else {
